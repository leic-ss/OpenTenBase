...
## 工作总结
本次工作，主要实现了一个可以输出trace报告的插件，插件的实现上，提供了一系列的API接口，通过这些API接口，实现的是一个类似开关的效果，打开开关后，Opentenbase中新增的钩子，会开启记录执行信息，输出到指定的trace报告文件中，关闭开关后，则停止记录执行信息，关闭执行报告文件。

## 后续工作
当前工作，基本上实现了一个完善的trace报告功能，但还存在若干不足，比如已trace的执行流程不够完全，执行信息不够完善，如果需要补充更完整的trace信息，或者需要新增部分定制化的trace信息，需要修改代码，重编编译发布，整个流程比较繁琐和低效。
在下一步工作中，我们希望重点解决该问题，
